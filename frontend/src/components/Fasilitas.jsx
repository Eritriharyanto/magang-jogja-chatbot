...
function Fasilitas() {
  const [fasilitas, setFasilitas] = useState(FASILITAS_FALLBACK.map((isi) => ({ isi })));

  useEffect(() => {
    let cancelled = false;
    getFasilitas()
      .then((data) => {
        if (!cancelled && Array.isArray(data) && data.length > 0) setFasilitas(data);
      })
      .catch(() => {
          ...
          {fasilitas.map((f, i) => (
            <Reveal key={f.id ?? f.isi} delay={i * 70} as="li">
              <div className="rounded-[1.75rem] border-b-[6px] border-[#7a4a4a] bg-mj-yellow px-6 py-3 text-center text-[0.95rem] font-bold leading-snug text-mj-ink transition-transform duration-300 hover:-translate-y-1 hover:shadow-lg">
                <span className="whitespace-pre-line">{f.isi}</span>
              </div>
            </Reveal>
          ))}
        </ul>
