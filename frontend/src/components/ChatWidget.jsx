...
function ChatWidget() {
  ...
  async function handleIdentitySubmit(e) {
    ...
    if (!/^[0-9+\s-]{8,15}$/.test(telepon)) {
      setIdentityError("Nomor WhatsApp gak valid, cek lagi ya (mis. 08123456789).");
      return;
    }
        ...
        <div className="flex h-[28rem] w-[20rem] flex-col overflow-hidden rounded-2xl bg-white shadow-2xl sm:w-[22rem]">
          ...
          {checkingIdentity ? (
            ...
            >
              ...
              {identityError ? (
                <p className="rounded-lg bg-red-50 px-3 py-2 text-xs text-red-600">{identityError}</p>
              ) : null}

              <div>
                <label className="mb-1 block text-xs font-semibold text-mj-ink" htmlFor="chat-nama">
                  Nama
                </label>
                <input
                  id="chat-nama"
              ...
              <div>
                <label className="mb-1 block text-xs font-semibold text-mj-ink" htmlFor="chat-telepon">
                  Nomor WhatsApp
                </label>
                <input
                  id="chat-telepon"
